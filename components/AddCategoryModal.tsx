...
interface AddCategoryModalProps {
  onClose: () => void;
  onAddCategory: (payload: CreateCategoryPayload) => void;
  isSubmitting: boolean;
}

const AddCategoryModal: React.FC<AddCategoryModalProps> = ({ onClose, onAddCategory, isSubmitting }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold mb-4 text-white">{t('modal_add_category_title')}</h2>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 mt-4">
            ...
            >
              {isSubmitting ? '...' : t('common_add_category')}
            </button>
          </div>
        </form>
      </div>
